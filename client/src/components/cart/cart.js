...
import { getLocalStorageItem, setLocalStorageItem, removeCartItemLocalStorage } from "../../utils/localStorage";
        ...
        setCartList((prevCartList) => {
          return prevCartList.map((cart) => {
            if (cart._id === cartId) {
              ...
              cart.total_price = cart.product[0].price * newQuantity;
            }
            return cart;
          });
        });
        setCart((prev) => prev + (newQuantity - cartList.find((c) => c._id === cartId).quantity));
      } catch (error) {
  ...
  const handleRemoveCart = async (cart) => {
    if (isAuthenticated) {
      try {
        ...
        setCartList((prevCartList) => {
          return prevCartList.filter((item) => item._id !== cart._id);
        });
      ...
      const localCart = [...cartList];
      const cartIndex = localCart.findIndex((item) => item.product.id === cart.product.id);
      if (cartIndex > -1) {
        removeCartItemLocalStorage(cart.product.id);
        const updatedCart = localCart.filter((item) => item.product.id !== cart.product.id);
        const totalQuantity = updatedCart.reduce((acc, item) => acc + item.quantity, 0);
        setCartList(updatedCart);
        setCart(totalQuantity);

        // close the modal
        const modalElement = document.getElementById(`deleteModal-${cart.product.id}`);
        const modal = new window.bootstrap.Modal(modalElement);
        const backdrop = document.querySelector('.modal-backdrop');
        modal.hide();
        if (backdrop) {
    ...
    <>
      {cartList.length > 0 ? (
              ...
              <div className={`${styles["cart_body"]} col-8`}>
                ...
                {isAuthenticated ? cartList.map((cart) => (
                      <div className="card rounded-3 mb-4" key={cart._id}>
                            ...
                            <div className="col-md-10 col-lg-10 col-xl-10 d-flex justify-content-between">
                              ...
                              <div className="text-end">
                                <p className="mb-0 text-danger">
                                  {formatCurrency(cart.product[0].price)}đ
                                </p>
                              </div>
                            </div>
                          </div>
                          <div className="d-flex justify-content-end">
                            <div className="">
                              <button
                                ...
                                data-bs-toggle="modal" data-bs-target={`#deleteModal-${cart._id}`}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  width="16"
                            ...
                            <div className="d-flex align-items-center">
                              ...
                              <input
                                ...
                                onChange={(e) => handleInputChange(cart._id, e.target.value)}
                              />
                              <button
                        ...
                        {/* Modal */}
                        <div className="modal fade" id={`deleteModal-${cart._id}`} tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div className="modal-dialog">
                            <div className="modal-content">
                              ...
                              <div className="modal-footer">
                                ...
                                onClick={() => handleRemoveCart(cart)}>Xoá</button>
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  : cartList.map((cart) => (
                      <div className="card rounded-3 mb-4" key={cart.product.id}>
                        <div className="card-body p-4">
                          ...
                          <div className="d-flex justify-content-end">
                            <div className="">
                              <button
                                className="btn text-danger"
                                data-bs-toggle="modal" data-bs-target={`#deleteModal-${cart.product.id}`}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  width="16"
                            ...
                            <div className="d-flex align-items-center">
                              <button
                                className={`${styles["minus_button"]} btn px-2`}
                                onClick={() => handleDecrement(cart.product.id, cart.quantity)}
                                disabled={cart.quantity <= 1}>
                                -
                              </button>
                              <input
                                ...
                                min="1"
                                className={`${styles["no-spinners"]} form-control form-control-sm text-center`}
                                onChange={(e) => handleInputChange(cart.product.id, e.target.value)}
                              />
                              <button
                                className={`${styles["plus_button"]} btn px-2`}
                                onClick={() => handleIncrement(cart.product.id, cart.quantity)}>
                                +
                              </button>
                            </div>
                          </div>
                        ...
                        {/* Modal */}
                        <div className="modal fade" id={`deleteModal-${cart.product.id}`} tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div className="modal-dialog">
                            <div className="modal-content">
                              <div className="modal-header">
                                <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                ...
                <div className={`card rounded-3 mb-4 p-3 text-start`}>
                  ...
                  <p className="d-flex mb-0 align-items-center justify-content-between fw-bold">
                    <span>Tổng tiền:</span>
                    <span>
                      {formatCurrency(cartList.reduce((total, item) => total + item.total_price,0))}đ
                    </span>
                  </p>
                  <hr />
                  <div>
        ...
        <div className={styles["cart-empty"]}>
          <img src="https://cdn.tgdd.vn/mwgcart/v2/vue-pro/img/empty-cart.22a86c722e7023e53b0dd30fe.png" className={styles["iconcart-empty"]} alt=""></img>
